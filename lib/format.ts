...
import type { Metric } from "./analytics/types";
...
export function formatMetric(metric: Metric): string {
  ...
  switch (metric.unit) {
    case "cents":   return money(Number(metric.value));
    case "percent": return percent(Number(metric.value));
    case "minutes": return minutes(Number(metric.value));
    case "people":
      return metric.key === "avg_party" ? decimal(Number(metric.value), 1) : integer(Number(metric.value));
    case "count":   return integer(Number(metric.value));
    case "ratio":   return decimal(Number(metric.value), 2);
